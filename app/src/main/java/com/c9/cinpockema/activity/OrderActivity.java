...
//                    tmp = response.toString();
                    ...
                    order.setCreatTime(response.substring(0,1));
                    order.setMovieName(response.substring(1,2));
                    order.setTotalPrice(response.substring(2,3));
    ...
    public ArrayList<Order> getMyselfList1() {
        ...
        order0.getCreatTime();
        order0.getMovieName();
        order0.getTotalPrice();
//        order0.getMovie().setSummary("对不起,你还没有任何订单!");
